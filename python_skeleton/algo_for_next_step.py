import eval7
import random
def algorithm(INTIMIDATE_PROB=1.0, INTIMIDATE_DEC=lambda x: 2**(-0.5 * x), RAISE_THRES=0.5, RAISE_RATIO=0.4):
    '''
    Parameters:
    ...
    def action(win_prob, current_pot, my_pot, continue_cost, raise_range):
        ...
        #we calculate intimidating as compared to the current poy
        intimidate_ratio = continue_cost / (current_pot - continue_cost)
        win_prob *= INTIMIDATE_DEC(intimidate_ratio)
